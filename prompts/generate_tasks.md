...
(若为"(无)"表示无额外要求;**若有具体要求,必须在任务设计中完全执行,优先级高于下面一切 habit / 原则 / 弱点偏好**)
</today_request 优先级最高>
...
规则(严格遵守):
1. **⚠️ 最高优先级:用户临时要求(`<today_request>`)**
   - 若内容非"(无)",**必须完全执行**,哪怕和 habit / 弱点排序 / 任务原则冲突
   - 示例:用户说"跳过听力/不做听力了" → 生成的任务里 **零条** module 含 听力/听写/听力词汇/语音纠正;用户说"只复盘错题" → 不生成新知识点任务;用户说"砍半" → 任务数减半
   - 至少 **1 张任务的 rationale_brief 明确引用用户原话**(让用户看到 AI 听见了),如"按你的要求不做听力,改为..."
   - 这是硬规则,哪怕规划里有听力 habit,也要按要求全砍掉;不按做等于失败
2. 覆盖今日预期时长的 80%-100%,留 20% 弹性
3. **total_time 硬上限**:所有任务 `estimated_minutes` 累加 **必须 ≤ `{today_hours} × 60` 分钟**,单张 10-240 分钟。
...
4. 优先修补**近 7 天跳过的任务**和**弱点排序靠前的模块**(规则 1 的 today_request 冲突时以 today_request 为准)
5. 考虑当前时段精力(下午适合输入型如听力阅读,晚上适合输出型如写作口语)
6. **按完成率动态调量**(context 里的 `overall_rate_pct` 和 `recent_7d_rate_pct`):
...
7. 不得编造用户状态;若 `last_7_days` 为"暂无",按新手保守节奏
8. **针对性复盘(硬规则)**:若 `<last_7_days>` 里存在:
   - `feeling ≤ 2` 的已完成任务(用户觉得吃力/挫败),或
   - `status=skipped` 的任务,
   今天至少生成 **1 张针对性复盘任务**(但若规则 1 的 today_request 与此冲突,以 today_request 为准)。title 格式建议 `复盘:<原任务简述>`。
   规则:
...
9. 只输出 JSON,不要任何额外文字或 markdown 代码块包裹
